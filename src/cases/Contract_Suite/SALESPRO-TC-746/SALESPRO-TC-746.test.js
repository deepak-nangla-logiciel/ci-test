...
} = require("../../../../utils/loginAndHomepage.js");
...
const { scrollToView } = require("../../../../utils/scrollToView.js");
...
describe("This suite will verify the Estimate cases.", () => {
  beforeEach(async () => {
...
it("should verify that the custom placeholder are pulling into Contracts.", async () => {
  await estimatesPage.addRoofingEstimate(
    estimateTestData.quantity,
    estimateName
  );
  await $(contractElements.leftNavigationContractsOption).waitForEnabled();
  await $(contractElements.leftNavigationContractsOption).click();
  await scrollToView(contractElements.windowTestContract);
  await $(contractElements.windowTestContract).waitForEnabled();
  await $(contractElements.windowTestContract).click();
  await $(contractElements.contractNextButton).click();
  //Assert that Shingles custom placeholder is visible
  const shinglesCustomPlaceholder = await $(
    estimateElements.customFieldShingles
  );
  await expect(await shinglesCustomPlaceholder.isDisplayed()).toBe(true);
  await $(contractElements.contractBackButton).waitForEnabled();
