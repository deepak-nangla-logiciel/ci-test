...
it("Verify that clicking clear button removes all estimate values.", async () => {
  // First add some quantities to the estimate
  await $(estimateElements.sidingCategory).waitForDisplayed();
  await $(estimateElements.sidingCategory).click();
  
  // Set some quantity values
  await $(estimateElements.selectQuantitySidingEstimate).waitForDisplayed();
  await $(estimateElements.selectQuantitySidingEstimate).setValue(estimateTestData.quantity);
  await $(estimateElements.doneButtonForKeypad).click();
  
  // Click the clear button
  await $(estimateElements.clearButton).waitForDisplayed();
  await $(estimateElements.clearButton).click();
  await $(estimateElements.confirmClearButton).waitForDisplayed();
  await $(estimateElements.confirmClearButton).click();
  
  // Assert that quantity is cleared
  const quantityValue = await $(estimateElements.selectQuantitySidingEstimate).getValue();
  expect(quantityValue).toBe(null); // Expect it to be empty

  // Navigate to Customers screen
  await $(estimateElements.backButtonEstimate).click();
  await $(estimateElements.confirmBackButtonEstimate).click();
});
});