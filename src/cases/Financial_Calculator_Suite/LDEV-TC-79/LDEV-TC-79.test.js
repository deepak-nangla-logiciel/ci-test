...
describe("LDEV-TC-79", () => {
  it("Estimates - Verify that user can Remove Finance Option", async () => {
    ...
    // Verify MSRP amount
    const msrpText = await $(financecalculatorElement.newTotalAmount);
    await expect(msrpText).toBeDisplayed();

    // Remove the finance option from the estimate
    await financneCalculatorPage.removeFinanceOptionFromEstimate();
    await expect(msrpText).not.toBeDisplayed();
  });
});
